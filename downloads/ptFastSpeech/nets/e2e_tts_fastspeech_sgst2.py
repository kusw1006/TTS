...
from nets.supervisedgst import GST

from espnet.espnet.asr.asr_utils import get_model_conf
from espnet.espnet.asr.asr_utils import torch_load
from espnet.espnet.nets.pytorch_backend.e2e_tts_transformer import TTSPlot
from nets.duration_calculator import (
    DurationCalculator,  # noqa: H301
)
from espnet.espnet.nets.pytorch_backend.fastspeech.duration_predictor import DurationPredictor
from espnet.espnet.nets.pytorch_backend.fastspeech.duration_predictor import (
    DurationPredictorLoss,  # noqa: H301
)
from espnet.espnet.nets.pytorch_backend.fastspeech.length_regulator import LengthRegulator
from espnet.espnet.nets.pytorch_backend.nets_utils import make_non_pad_mask
from espnet.espnet.nets.pytorch_backend.nets_utils import make_pad_mask
from espnet.espnet.nets.pytorch_backend.tacotron2.decoder import Postnet
from espnet.espnet.nets.pytorch_backend.transformer.attention import MultiHeadedAttention
from espnet.espnet.nets.pytorch_backend.transformer.embedding import PositionalEncoding
from espnet.espnet.nets.pytorch_backend.transformer.embedding import ScaledPositionalEncoding
from espnet.espnet.nets.pytorch_backend.transformer.encoder import Encoder
from espnet.espnet.nets.pytorch_backend.transformer.initializer import initialize
from espnet.espnet.nets.tts_interface import TTSInterface
from espnet.espnet.utils.cli_utils import strtobool
from espnet.espnet.utils.fill_missing_args import fill_missing_args
...
class FeedForwardTransformer(TTSInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        # Style related
        group.add_argument('--use-gst', default=False, type=strtobool,
                           help='Whether to use style modeling')
        # Reference Encoder
        group.add_argument('--refer-filters', default=[32, 32, 64 ,64, 128, 128], type=int, nargs='+',
                           help='Filter size of reference encoder')
        # Global Style Token
        group.add_argument('--gru-unit', default=128, type=int,
                           help='Number of gru unit')
        group.add_argument('--num-gst', default=10, type=int,
                           help='Number of global style tokens')
        group.add_argument('--num-heads', default=4, type=int,
                           help='Number of heads in Multi-head attention')
        group.add_argument('--style-dim', default=10, type=int,
                           help='Dimension of style embedding')
        group.add_argument("--sty-embed-integration-type", type=str, default="add",
                           choices=["add", "concat"],
                           help="How to integrate speaker embedding")
        # Speaker ID
        group.add_argument('--num-spk', default=0, type=int,
                           help='Number of global style tokens')
        return parser
    ...
    def __init__(self, idim, odim, args=None):
        ...
        self.use_speaker_id = args.use_speaker_id
        if self.spk_embed_dim is not None:
            self.spk_embed_integration_type = args.spk_embed_integration_type
        if self.use_gst:
            self.sty_embed_integration_type = args.sty_embed_integration_type
        ...
        if self.spk_embed_dim is not None:
            if self.spk_embed_integration_type == "add":
                self.spk_projection = torch.nn.Linear(self.spk_embed_dim, args.adim)
            else:
                self.spk_projection = torch.nn.Linear(args.adim + self.spk_embed_dim, args.adim)

        if self.use_gst:
            if self.sty_embed_integration_type == "add":
                self.sty_projection = torch.nn.Linear(args.style_dim, args.adim)
            else:
                self.sty_projection = torch.nn.Linear(args.adim + args.style_dim, args.adim)

        if self.use_speaker_id:
            self.spk_embed_lookup = torch.nn.Embedding(
                num_embeddings=args.num_spk,
                embedding_dim=args.spk_embed_dim,
                padding_idx=padding_idx)
        ...
        if self.use_gst:
            self.gst = GST(idim=odim, odim=args.style_dim, rlayers=args.refer_filters, gru_unit=args.gru_unit,
                           num_gst=args.num_gst, num_heads=args.num_heads)

    def _forward(self, xs, ilens, ys=None, olens=None, spembs=None, stembs=None, ds=None, is_inference=False, num_emotion=None):
        # forward encoder
        ...
        if self.use_speaker_id:
            if is_inference:
                spembs = self.spk_embed_lookup(spembs.squeeze(1))
            else:
                spembs = self.spk_embed_lookup(spembs)
        ...
        if self.use_gst:
            if ys is not None:
                y_stembs = self.gst(ys, num_emotion)
            elif stembs is not None:
                y_stembs = stembs
            hs = self._integrate_with_sty_embed(hs, y_stembs)
    ...
    def forward(self, xs, ilens, ys, olens, spembs=None, emembs=None, stembs=None, extras=None,  *args, **kwargs):
        """Calculate forward propagation.
        ...
        before_outs, after_outs, ds, d_outs = self._forward(
            xs, ilens, ys, olens, spembs=spembs, stembs=stembs, ds=extras, is_inference=False, num_emotion=emembs)
    ...
    def calculate_all_attentions(self, xs, ilens, ys, olens, spembs=None, emembs=None, extras=None, *args, **kwargs):
        ...
        with torch.no_grad():
            ...
            # forward propagation
            outs = self._forward(xs, ilens, ys, olens, spembs=spembs, num_emotion=emembs, ds=extras, is_inference=False)[1]
    ...
    def inference(self, x, y, inference_args, spemb=None, stemb=None, ememb=None, *args, **kwargs):
        ...
        if self.use_gst is not None:
            if y is not None:
                ys = y.unsqueeze(0)
                stembs = None
            elif stemb is not None:
                ys = None
                stembs = stemb.unsqueeze(0)



        # inference
        _, outs, _ = self._forward(xs, ilens, ys, spembs=spembs, stembs=stembs, is_inference=True, num_emotion=ememb)  # (1, L, odim)
    ...
    def _integrate_with_sty_embed(self, hs, stembs):
        """Integrate speaker embedding with hidden states.
        ...
        if self.sty_embed_integration_type == "add":
            # apply projection and then add to hidden states
            spembs = self.sty_projection(stembs)
            hs = hs + spembs.unsqueeze(1) 
        elif self.sty_embed_integration_type == "concat":
            # concat hidden states with spk embeds and then apply projection
            spembs = F.normalize(stembs).unsqueeze(1).expand(-1, hs.size(1), -1)
            hs = self.sty_projection(torch.cat([hs, spembs], dim=-1))
        else:
            raise NotImplementedError("support only add or concat.")
