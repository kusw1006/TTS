...
class GST(nn.Module):
    ...
    def forward(self, inputs, weight=1.0):
        enc_out = self.encoder(inputs) # batch, 128
        style_embed = self.stl(enc_out, weight) # [N, 1, num_units]
...
class STL(nn.Module):
    ...
    def __init__(self, num_gst=10, style_dim=80, gru_unit=80, num_heads=8):
        ...
        self.embed = nn.Parameter(torch.FloatTensor(num_gst, key_dim))
    ...
    def forward(self, inputs, weight=1.0):
        N = inputs.size(0)
        query = inputs.unsqueeze(1) * weight  # [N, 1, E//2]
        keys1 = self.embed #* weight
        keys = torch.tanh(keys1).unsqueeze(0).expand(N, -1, -1) #* weight # [N, token_num, E // num_heads]
        style_embed = self.attention(query, keys)

        return style_embed #* weight #* weight # [N, 1, num_units]
