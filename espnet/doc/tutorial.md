...
criterion: loss
mtlalpha: 1.0
model-module: "espnet.espnet.nets.pytorch_backend.e2e_asr_transducer:E2E"
```
...
From espnet v0.4.0, we have three options in `--batch-count` to specify minibatch size (see `espnet.espnet.utils.batchfy` for implementation);
