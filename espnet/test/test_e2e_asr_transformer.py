...
def test_sequential():
    ...
    from espnet.espnet.nets.pytorch_backend.transformer.repeat import MultiSequential
...
def test_mask(module):
    T = importlib.import_module(
        "espnet.espnet.nets.{}_backend.e2e_asr_transformer".format(module)
    )
...
def prepare(backend, args):
    ...
    odim = 5
    T = importlib.import_module(
        "espnet.espnet.nets.{}_backend.e2e_asr_transformer".format(backend)
    )
...
def test_transformer_trainable_and_decodable(module, model_dict):
    ...
    if module == "pytorch":
        ...
        from espnet.espnet.nets.pytorch_backend.transformer import plot
...
def prepare_copy_task(d_model, d_ff=64, n=1):
    T = importlib.import_module("espnet.espnet.nets.pytorch_backend.e2e_asr_transformer")
    idim = 11
    odim = idim
